package com.c1ok.bedwars.internal.bedwars.exp
...
import com.c1ok.bedwars.internal.bedwars.*
import com.c1ok.bedwars.internal.bedwars.bounds.GameDataCleaner
import com.c1ok.bedwars.internal.bedwars.bounds.GeneratorBound
import com.c1ok.bedwars.internal.bedwars.bounds.PlayerTeleport
import com.c1ok.bedwars.internal.bedwars.exp.gen.Iron
import com.c1ok.bedwars.internal.bedwars.item.SpecialItemManagerImpl
import com.c1ok.bedwars.internal.bedwars.item.items.ExitTeam
import com.c1ok.bedwars.internal.bedwars.item.items.SelectTeam
...
class ExpBedWars: BaseBedWarsGame("test", 64, 2, Pos(-190.0, 58.0, 463.0)) {
    ...
    override fun init() {
        val first = isFirstInit
        super.init()
        if (first) {
            specialManager.addSpecial(SelectTeam)
            specialManager.addSpecial(ExitTeam)
        }
    }
    ...
    override fun addPlayerResource(player: IBedWarsPlayer, resource: String, count: Number) {
        if (resource != "level") return
        val minePlayer = player.miniPlayer.player ?: return
    ...
    override fun removePlayerResource(player: IBedWarsPlayer, resource: String, count: Number) {
        val minePlayer = player.miniPlayer.player ?: return
        minePlayer.level -= count.toInt()
    }
