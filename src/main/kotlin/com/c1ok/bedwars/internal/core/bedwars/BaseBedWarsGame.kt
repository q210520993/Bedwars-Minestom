...
import com.c1ok.bedwars.internal.core.bedwars.bounds.GameDataCleaner
...
abstract class BaseBedWarsGame(
    ...
    override fun init(): CompletableFuture<Boolean> {
        ...
        val future = CompletableFuture.completedFuture(isFirstInit)
    ...
    override fun getBedwarsPlayer(uuid: UUID): IBedWarsPlayer? {
        return gamePlayers[uuid]
    }
    ...
    override fun rebuild(): CompletableFuture<Void> {
        bounds.forEach {
            it.onRebuild()
        }
        return CompletableFuture.completedFuture(null)
